...
扩展弹窗会自动检查版本。发现新版后可直接点击“下载更新”；下载完成后解压覆盖旧扩展目录，在扩展管理页点击“重新加载”，最后刷新已经打开的微存网页。开发者模式加载的解压扩展受 Chromium 安全限制，不能静默覆盖自身文件。
...
图片 OCR 默认开启。扩展把微博图片临时交给当前微存页面，Tesseract.js 在浏览器内完成识别；图片不会发送到微存服务器，也不会作为二进制写入归档。IndexedDB 只保存原图地址、图片在正文中的位置和识别文字。首次识别会下载中文与英文识别模型，之后浏览器会复用缓存。阅读页可以对失败图片重试，也可以为旧归档重新读取图片并补做 OCR。正文直接加载原图失败时会自动改由扩展读取，“查看原图”也通过同一条受限图片通道打开。为兼容新浪图片防盗链，扩展只在读取目标图片期间临时附加微博来源请求头，完成后立即删除该规则。

## Cookie

公开文章通常无需填写凭据。需要登录查看的内容会优先使用扩展检测到的当前微博登录状态。

- 扩展会自动检测当前浏览器已有的微博登录 Cookie，并优先在已打开的微博标签页中读取微博自身的用户状态；正文请求也优先在微博第一方页面环境中发出，让浏览器按原规则携带 Cookie。没有可复用页面时会短暂打开后台标签页并在请求后关闭；扩展后台附加 Cookie 的请求仅作为页面执行失败时的备用路径，会话规则在单次请求后立即删除。
- 手动填写 Cookie 仅作为未在当前浏览器登录微博时的备用方式。
- 微博页面已确认登录或登录状态暂时无法确认时，扩展不会用本地保存的手动 Cookie 覆盖浏览器现有 Cookie。
- 登录验证结果缓存在当前浏览器会话中；打开扩展弹窗只读取缓存，只有手动点击“刷新状态”或核心登录 Cookie 改变时才重新验证。
...
- 浏览器本地 OCR 仅适合辅助检索，可能出现错字或遗漏；中文长图、小字和复杂排版尤其容易不准确，请以原图为准。识别速度取决于设备性能和图片数量，超过 10 MB 的单张图片会跳过。
- 图片二进制不会写入备份，阅读页展示原图时仍需要联网并保持扩展连接。
