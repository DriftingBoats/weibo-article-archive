...
网站没有服务器、账号系统或云端数据库。文章、章节、访问 Token 和 Cookie 都保存在当前浏览器；抓取扩展只负责绕过网页无法跨域读取微博的限制，解析、搜索、阅读和导出仍由前端完成。
...
- 全部归档备份与恢复；备份默认不包含 Token/Cookie
- 响应式界面、键盘焦点与减少动态效果支持
...
公开文章通常无需填写凭据。需要登录查看的内容可以在首页“本地访问设置”中保存 Token 或 Cookie。

- 扩展会自动检测并使用当前浏览器已有的微博 Cookie；无需复制 Cookie。
- 手动填写 Cookie 仅作为未在当前浏览器登录微博时的备用方式。
- 两者都写入当前网站源的 IndexedDB。
- 微存没有后端，因此不会上传到微存服务器。
...
不要在 issue、日志或截图中公开 Cookie 和 Token。安全问题请按照 [SECURITY.md](SECURITY.md) 私下报告。
